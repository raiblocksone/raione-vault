...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  remoteVisible = false;
  blockTypes: string[] = ['Send RaiblocksOne', 'Change Representative'];
  ...
  async receiveReceivableBlock(receivableBlock) {
    ...
    if (createdReceiveBlockHash) {
      ...
      this.notifications.sendSuccess(`Successfully received RaiblocksOne!`, { identifier: 'success-receive' });
  ...
  async generateSend() {
    ...
    if (!this.validateAmount()) return this.notifications.sendWarning(`Invalid XRO Amount`);
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notifications.sendError(`From account does not have enough XRO`);
    ...
    const blockData = {
      account: this.accountID.replace('nano_', 'xro_').toLowerCase(),
    ...
    const blockDataPrevious = {
      account: jsonBlock.account.replace('nano_', 'xro_').toLowerCase(),
