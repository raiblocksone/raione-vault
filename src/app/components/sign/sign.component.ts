...
export class SignComponent implements OnInit {
  ...
  signTypeChange() {
    ...
    switch (this.signTypeSelected) {
      ...
      case this.signTypes[0]:
        this.walletAccount = this.accounts.find(a => a.id.replace('nano_', 'xro_') === this.signatureAccount);
  ...
  async prepareTransaction() {
    ...
    if (this.txType === TxType.send || this.txType === TxType.change) {
      this.signatureAccount = this.fromAccountID.replace('nano_', 'xro_').toLowerCase();
    } else if (this.txType === TxType.receive || this.txType === TxType.open) {
      this.signatureAccount = this.toAccountID.replace('nano_', 'xro_').toLowerCase();
    }
