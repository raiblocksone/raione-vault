...
export class ReceiveComponent implements OnInit, OnDestroy {
  ...
  async changeQRAccount(account) {
    ...
    if (account.length > 1) {
      ...
      this.qrCodeUri = `xro:${account}${this.qrAmount ? `?amount=${this.qrAmount.toString(10)}` : ''}`;
  ...
  async changeQRAmount(raw?) {
    ...
    if (this.qrAccount.length > 1) {
      this.qrCodeImage = null;
      this.qrCodeUri = `xro:${this.qrAccount}${this.qrAmount ? `?amount=${this.qrAmount.toString(10)}` : ''}`;
      qrCode = await QRCode.toDataURL(this.qrCodeUri, {scale: 7});
      this.qrCodeImage = qrCode;
    }
  }
  ...
  async receiveReceivableBlock(receivableBlock) {
    ...
    if (createdReceiveBlockHash) {
      ...
      this.notificationService.sendSuccess(`Successfully received RaiblocksOne!`, { identifier: 'success-receive' });
