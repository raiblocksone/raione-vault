...
export class AppComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    // New for v19: Patch saved xro_ prefixes to nano_
    ...
    // Workaround fix for github pages when RaiOne-Vault is refreshed (or externally linked) and there is a subpath for example to the send screen.
    ...
    this.updates.activated.subscribe((event) => {
      ...
      this.notifications.sendSuccess('RaiOne-Vault was updated successfully.');
    });
  ...
  /*
    This is important as it looks through saved data using hardcoded xro_ prefixes
  ...
  async patchXrbToNanoPrefixData() {
    ...
    await this.walletService.patchOldSavedData(); // Change saved xro_ addresses to nano_
  ...
  performSearch() {
    ...
    const isValidNanoAccount = (
        ( searchData.startsWith('xro_') || searchData.startsWith('xrb_') || searchData.startsWith('nano_') )
    ...
    this.notifications.sendWarning(`Invalid RaiblocksOne address or block hash! Please double check your input`);
  }
  ...
  retryConnection() {
    ...
    this.notifications.sendInfo(`Attempting to reconnect to RaiblocksOne node`);
  }
