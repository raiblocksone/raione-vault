...
export class MusigService {
  ...
  aggregate(storedAccounts, runWithPubkeys = null) {
    ...
    for (let address of addresses) {
      ...
      if (!address.startsWith('xro_') && !address.startsWith('xro_') && !address.startsWith('nano_')) {
        throw new Error('RaiblocksOne addresses should start with xro_');
      }
      address = address.split('_', 2)[1];
      try {
        const bytes = base32.decode(address);
        if (bytes.length !== 37) {
          throw new Error('Wrong RaiblocksOne address length');
        }
        ...
        if (!this.util.array.equalArrays(bytes.subarray(32), checksum_)) {
          throw new Error('Invalid RaiblocksOne address checksum');
        }
        pubkeys.push(pubkey);
      } catch (err_) {
          console.error(err_.toString());
          throw new Error('Invalid RaiblocksOne address (bad character?)');
      }
    }
    ...
    const fullAddressFinal = 'xro_' + base32.encode(fullAddress);
